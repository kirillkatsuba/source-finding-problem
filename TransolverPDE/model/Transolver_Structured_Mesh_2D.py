...
from TransolverPDE.model.Embedding import timestep_embedding
from TransolverPDE.model.Physics_Attention import Physics_Attention_Structured_Mesh_2D
...
class Model(nn.Module):
    ...
    def get_grid(self, batchsize=1):
        ...
        grid = torch.cat((gridx, gridy), dim=-1)  # .to(torch.device("cpu"))  # B H W 2

        gridx = torch.tensor(np.linspace(0, 1, self.ref), dtype=torch.float)
        gridx = gridx.reshape(1, self.ref, 1, 1).repeat([batchsize, 1, self.ref, 1])
        gridy = torch.tensor(np.linspace(0, 1, self.ref), dtype=torch.float)
        gridy = gridy.reshape(1, 1, self.ref, 1).repeat([batchsize, self.ref, 1, 1])
        grid_ref = torch.cat((gridx, gridy), dim=-1)  # .to(torch.device("cpu")) # B H W 8 8 2
