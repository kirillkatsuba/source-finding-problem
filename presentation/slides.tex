...
\input{metrics.tex}   % макросы метрик, генерит gen_metrics.py из experiments/*/metrics.json
...
\begin{frame}{Функция потерь}
  ...
  \begin{itemize}
    \item \textbf{field} --- реконструкция матрицы $C_{t=0}$
    \item \textbf{heatmap} --- spatial softmax, матрица вероятностей источника
  ...
  \[
    ...
    \mathcal{L}_{\text{coord}}=\frac{1}{2}\lVert\hat{x}-x\rVert_2^2
  \]
\end{frame}

\begin{frame}{Случайный сдвиг источника}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_augment.png}
\end{frame}
...
\begin{frame}{Тривиальный бейзлайн --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_trivial.png}
\end{frame}
...
\begin{frame}{Физический бейзлайн --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_physical.png}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[node distance=2.4mm]
        ...
        \node[vbb, below=of a](b){Transolver $\to$ $\hat{C}_{t=0}$};
        \node[vio, below=of b](c){source $=\arg\max$};
        \draw[var](a)--(b); \draw[var](b)--(c);
      \end{tikzpicture}
    \end{column}
...
\begin{frame}{Transolver (только поле) --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_field_only.png}
\end{frame}

\begin{frame}{Transolver + регрессия --- архитектура}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      \textbf{Функция потерь:}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[node distance=2.4mm]
        ...
        \node[vop, below=of d](e){AdaptiveAvgPool2d(8) $\to$ Linear $128$ $\to$ Linear $2$};
        \node[vio, below=of e](f){source $=(\hat x, \hat y)$};
...
\begin{frame}{Transolver + регрессия --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_regressor.png}
\end{frame}
...
\begin{frame}{Transolver + heatmap --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_heatmap.png}
\end{frame}
...
\begin{frame}{UNet + heatmap --- архитектура}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[node distance=2.4mm]
        \node[vio](a){$[C_{t=1}, \dots, C_{t=17}]$ \ ($17{\times}H{\times}W$)};
        \node[vop, below=of a](b){Conv $3{\times}3$,\ $32\!\to\!64\!\to\!128$,\ GN+GELU, pool};
        \node[vop, below=of b](c){Conv $3{\times}3$,\ $128\!\to\!256$,\ GN+GELU};
        \node[vop, below=of c](d){Conv $3{\times}3$,\ $256\!\to\!128\!\to\!64\!\to\!32$,\ GN+GELU, upconv$+$skip};
        \node[vop, below=of d](e){Conv $1{\times}1$ $\to$ матрица вероятностей};
...
\begin{frame}{UNet + heatmap --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_unet.png}
\end{frame}
...
\begin{frame}{Transolver + heatmap + ветер --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_wind.png}
\end{frame}
...
\begin{frame}{PINN --- архитектура}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      ...
      \textbf{Ошибка на тесте}:
      \begin{itemize}
        \item без сглаживания --- с физ. лоссом \textbf{\mPinnMean}, без \textbf{\mPinnNoPdeMean}
        \item со сглаживанием --- с физ. лоссом \textbf{\mPinnSmooth}, без \textbf{\mPinnNoPdeSmooth}
      \end{itemize}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[node distance=2.4mm]
        ...
        \node[vop, below=6mm of c, xshift=-1.4cm, text width=2.2cm](hf){Conv $1{\times}1 \to \hat C_{t=0}$};
        \node[vop, below=6mm of c, xshift=1.4cm, text width=2.2cm](hh){Conv $1{\times}1 \to$ матрица вероятностей};
        \node[vio, below=of hh, text width=2.2cm](e){source $=\arg\max$};
        \draw[var](a)--(b); \draw[var](b)--(c);
        \draw[var](c)--(hf); \draw[var](c)--(hh);
        \draw[var](hh)--(e);
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{PINN --- примеры}
  \vspace{0.4em}
  \centering\includegraphics[width=1\textwidth]{demo_pinn.png}
\end{frame}
...
\begin{frame}{Сравнение методов --- ошибка локализации (клетки)}
  ...
  {\scriptsize
    \begin{itemize}
      ...
      \item Предсказывать матрицу вероятностей лучше, чем делать регрессию координат
      \item При наличии ветра и меньшем объёме данных, можно получить сопоставимый с большим объёмом данных результат
    \end{itemize}
  }
